...
pub fn encrypt(receiver_pub: &[u8], msg: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    let aes_key = encapsulate(&ephemeral_sk, &receiver_pk, is_hkdf_key_compressed())?;
    let encrypted = sym_encrypt(&aes_key, msg).ok_or(Error::InvalidMessage)?;

    let mut cipher_text = Vec::with_capacity(get_ephemeral_key_size() + encrypted.len());
    let ephemeral_pk = pk_to_vec(&ephemeral_pk, is_ephemeral_key_compressed());
...
pub fn decrypt(receiver_sec: &[u8], msg: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    let aes_key = decapsulate(&ephemeral_pk, &receiver_sk, is_hkdf_key_compressed())?;
